...
// GET /movies (List semua film dengan nama sutradara)
...
// POST /movies (Tambah film, memerlukan autentikasi)
...
// PUT /movies/:id (Update film, memerlukan autentikasi dan peran admin)
...
// DELETE /movies/:id (Hapus film, memerlukan autentikasi dan peran admin)
app.delete("/movies/:id", [authenticateToken, authorizeRole("admin")], async (req, res, next) => {
  ...
  try {
    ...
    res.status(204).send(); // 204 No Content untuk operasi penghapusan berhasil
  } catch (err) {
    next(err);
  }
});
...
// Area ini dikosongkan sesuai catatan tugas praktikum.

// === FALLBACK & ERROR HANDLING ===

// Middleware 404 (Rute tidak ditemukan)
app.use((req, res) => {
